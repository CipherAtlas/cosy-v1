...
parser=argparse.ArgumentParser();parser.add_argument('--port',type=int,default=3011);args=parser.parse_args()
...
allowed.update({'villager-dialogue.json', 'villager-approach.json', 'villager-approach-runtime.json'})
class Handler(SimpleHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  (evidence/name).write_bytes(self.rfile.read(size));self.send_response(200);self.end_headers();self.wfile.write(b'Saved')
